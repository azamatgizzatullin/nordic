...
module.exports = {
    'GET': (res, req) => {
        console.log('3 Этап: GET_COMMENT')
    ...
    'GET_ALL': (res, req) => {
        ...
        console.log('Сделал запрос к БД');
    ...
    'GET_ALL': (res, req) => {
        console.log('3 Этап: Роут get_all_comments')
    ...
    'ADD': (res, req) => {
         ...
         */
        console.log('3 Этап: Роут add_comment')
        ...
        (id, text, user_id, time)
            VALUES
        ('${dataArray.id}', '${dataArray.text}', '${dataArray.user_id}', '${dataArray.time}')    
        `
        console.log(sql)
        const requestMessage = {
            STATUS_CODE: 200,
            REQUEST_TEXT: `Комментарий с id ${dataArray.id} успешно добавлен`,
            ID: dataArray.id
        }
